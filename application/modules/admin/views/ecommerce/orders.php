...
if (!isset($_GET['settings'])) {
    ...
    <td colspan="2">
        ...
        $total_amount = 0; // Move the initialization outside the loop
        foreach ($arr_products as $product) {
            // Calculate the total amount for each product
            $price = str_replace(' ', '', str_replace(',', '.', $product['product_info']['price']));
            ...
            <div style="word-break: break-all;">
                ...
                <div class="">
                    <b>Vendor:</b>
                    <a href="<?= base_url('admin/listdelivery?id=' . $product['product_info']['vendor_id']) ?>"><?= isset($product['product_info']['vendor_name']) ? $product['product_info']['vendor_name'] : '-Vendor name is missing-' ?></a>
                </div>
                <div class="clearfix"></div>
                <div style="padding-top:10px; font-size:16px;">Total amount for this product: <?= $product_amount.' '.$this->config->item('currency') ?></div>
                <hr>
            </div>

        <?php }
        ?>
        <!-- Display the total amount outside the loop -->
